"""Test suite for pandas propagate_entity_ids function.

Example test that can be easily copied and edited.
"""

import pandas as pd
import pytest

from pts.pyspark.facets.pandasprop import propagate_entity_ids
...
def test_propagate_from_dict():
    ...
    # Run propagation
    result, iterations = propagate_entity_ids(df, return_iterations=True)
    ...
    assert pd.isna(grandparent1_row['parent_id']) or grandparent1_row['parent_id'] is None
...
def test_propagate_multiple_parents():
    ...
    parent1_row = next(r for r in result_list if r['id'] == 'parent1' and pd.isna(r['parent_id']))
    parent2_row = next(r for r in result_list if r['id'] == 'parent2' and pd.isna(r['parent_id']))
