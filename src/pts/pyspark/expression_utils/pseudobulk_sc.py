...
class PseudobulkExpression:
    ...
    def filter_anndata(self, min_cells=0, min_genes=0, technology='10X'):
        ...
        self.adata = self.adata[self.adata.obs['method'] == technology].copy()
    ...
    def normalise_counts(self, counts_vector: pd.Series) -> pd.Series:
        """Normalise a vector of summed counts using CPM method.

        Args:
            counts_vector (pd.Series): Vector of summed counts to normalize

        Returns:
            pd.Series: Normalized counts vector (CPM)
        """
    ...
    def _save_chunk(self, df: pd.DataFrame, output_dir: str):
        ...
        if self.json:
            raw_sdf.write.mode('append').json(output_dir)
        else:
            # If not JSON, write as parquet
            raw_sdf.write.mode('append').parquet(output_dir)
    ...
    def run(
        ...
        if tissue_agg_colname:
            aggregation_configs.append({'tissue_agg_colname': tissue_agg_colname, 'celltype_agg_colname': None})
        if celltype_agg_colname:
            aggregation_configs.append({'celltype_agg_colname': celltype_agg_colname, 'tissue_agg_colname': None})
        if tissue_agg_colname and celltype_agg_colname:
            aggregation_configs.append({
                'tissue_agg_colname': tissue_agg_colname,
