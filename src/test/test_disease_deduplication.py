...
class TestDiseaseDeduplication:
    ...
    def _setup(self: TestDiseaseDeduplication) -> None:
        ...
        self.disease_df = pl.read_parquet(f'{script_path}/test_data/test_disease_w_duplication.parquet')
    ...
    def test_columns_propagated(self: TestDiseaseDeduplication, column_name: str) -> None:
        ...
        # Get dropped value:
        depricated_disase_values = self.disease_df.filter(pl.col('id') == dropped_id).to_dicts()[0][column_name]

        # Get the row merged, test if exists:
        current_disase_values = (
        ...
        for x in depricated_disase_values:
            assert x in current_disase_values
